...
const Login = () => {
  ...
  const socialLogin = async (provider) => {
    try {
      window.location.href = `${VITE_API_URL}/oauth2/authorization/${provider}`;
    } catch (error) {
    ...
    <>
      {splash ? (
        ...
        <L.LoginPage>
          <L.Logo src={logo} />
          <L.Login>
            <L.SocialLogin style={{ backgroundColor: '#FEE500' }} onClick={() => socialLogin('kakao')}>
              <L.SocialLogo src={kakao} />
              <L.SocialText>카카오로 시작하기</L.SocialText>
            </L.SocialLogin>
            <L.SocialLogin style={{ backgroundColor: '#04C75B' }} onClick={() => socialLogin('naver')}>
              <L.SocialLogo src={naver} />
              <L.SocialText>네이버로 시작하기</L.SocialText>
            </L.SocialLogin>
            <L.SocialLogin style={{ backgroundColor: '#FFF' }} onClick={() => socialLogin('google')}>
